...
const fetchServices = async () => {
  ...
  try {
    ...
    // 如果serviceContent有值，转换为serviceContentId
    if (params.serviceContent) {
      const content = contentOptions.value.find(item =>
          item.label === params.serviceContent
      );
      if (content) {
        params.serviceContentId = content.value;
      }
      delete params.serviceContent; // 删除serviceContent参数
    }
    ...
    if (response.code === 200 && response.data) {
      ...
      const records = (data.records || []).map(record => {
        const serviceTypeObj = typeOptions.value.find(option =>
            String(option.value) === String(record.serviceTypeId)
        );

        const serviceContentObj = contentOptions.value.find(option =>
            Number(option.value) === Number(record.serviceContentId)
        );

        return {
          ...record,
          serviceType: serviceTypeObj ? serviceTypeObj.label : `未知类型(ID:${record.serviceTypeId})`,
          serviceContent: serviceContentObj ? serviceContentObj.label : record.serviceContent || `未知内容(ID:${record.serviceContentId})`,
        };
      });
...
const submitAddService = async (formData) => {
  try {
    ...
    const submitData = {
      ...formData,
      serviceContentId: null // 根据实际情况设置或从选项中找到对应的ID
    };

    // 如果服务内容是从选项中选择的，找到对应的ID
    if (formData.serviceContent) {
      const content = contentOptions.value.find(item =>
          item.label === formData.serviceContent
      );
      if (content) {
        submitData.serviceContentId = content.value;
      }
    }
...
const editServiceHandle = (service) => {
  // 找到对应的服务内容文本
  let serviceContentText = service.serviceContent;
  if (!serviceContentText) {
    const contentObj = contentOptions.value.find(item =>
        Number(item.value) === Number(service.serviceContentId)
    );
    serviceContentText = contentObj ? contentObj.label : '';
  }

  Object.assign(editServiceData, {
    id: service.id,
    serviceTypeId: service.serviceTypeId,
    serviceContent: serviceContentText, // 使用文本内容
